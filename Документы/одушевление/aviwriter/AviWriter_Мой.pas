...
y:=360;
di:=+1;
while (tAvi>=0){(i<=qm)}and not stop do begin with m[i] do begin
  if sel then begin
    dec(tAvi,iAvi);
    wl1(i,form2.label1);
    if uwn>0 then begin wuw:=uwn; smx:=sxn; smy:=syn end
             else begin wuw:=0; smx:=0; smy:=0 end;
    uGamma(i);
    kFil:=true;
//Получение картинки из основной программы Enjol
    pok(i,0,0,x-1,y-1);
    sbrosGamma;
    inc(pBar); bar(form2.image1,pBar,maxAvi);
             try
               BitmapInfo := nil;
               BitmapBits := nil;
...
{             ExtBitmap :=b;// Bitmaps[0];
              if fStretch
               ...
               AVIERR :=
                  AVIStreamWrite(pStream, j, 1, BitmapBits, BitmapSize, AVIIF_KEYFRAME,
...
inc(i,di);
if i>qm then if o.ww then begin i:=qm;di:=-di end else i:=1;
if i=0  then if o.ww then begin i:=1; di:=-di end else i:=qm;
...
form2.panel1.visible:=false;           application.processMessages;
form2.label1.caption:=t('еще немного');application.processMessages;
end;
